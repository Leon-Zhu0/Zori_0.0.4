        ...
        if (!this.holes.isEmpty() && Wrapper.getPlayer() != null && Wrapper.getWorld() != null) {
            ...
            this.holes.forEach((blockPos, isBedrock) -> {
                if (this.sync.getValue()) {
                    ...
                    RenderUtils.INSTANCE.drawBoundingBox(blockPos, c.getRed() / 255.0f, c.getGreen() / 255.0f, c.getBlue() / 255.0f, 0.5f, this.linewidth.getValue());
                    RenderUtils.INSTANCE.drawBox(blockPos, c.getRed() / 255.0f, c.getGreen() / 255.0f, c.getBlue() / 255.0f, 0.22f);
                }
                else {
                    if (isBedrock) {
